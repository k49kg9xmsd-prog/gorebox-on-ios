...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "BootstrapPending") {
            ...
            showMessage(title: "上一個 Bootstrap 未完成", message: "上次執行完整 ELF bootstrap 後沒有回寫完成標記，代表 App 很可能在某個完整 Android binary call 被終止。重新跑前可以先看全面診斷。")
        }
    }
    ...
    @objc private func runBootstrap() {
        ...
        let alert = UIAlertController(
            title: "實驗性啟動 GoreBox",
            message: "這次不是單一 ARM64 ret 測試。Runner 會載入完整 libmain / RayFire / IL2CPP / Unity ELF，實際套 relocation + compatibility shim，並執行受控的 libmain JNI_OnLoad 與完整 RayFire image。\n\n若某個真正的 Android binary call 在目前環境出錯，App 仍可能被系統關閉。",
