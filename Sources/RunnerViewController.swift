...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "GraphicsProbePending") {
            ...
            showMessage(title: "上一個 Unity 圖形 Probe 未完成", message: "最後 checkpoint：\n\n\(cp)\n\n如果 libunity JNI_OnLoad 被終止，這裡會精確顯示是在 ELF load、JNI_OnLoad 或 RegisterNatives 哪一步。")
        }
    }
    ...
    private func configureUI() {
        ...
        styleButton(graphicsButton, title: "Unity 圖形橋 + JNI 註冊測試", action: #selector(runGraphicsBridge))
    ...
    private func refreshInstalledState() {
        ...
        if installed, let meta = ImportedGame.metadata() {
            ...
            textView.text = "遊戲已安裝。\n\n底層 ELF / relocation / libmain JNI_OnLoad 已通過。現在可以按「Unity 圖形橋 + JNI 註冊測試」：它會建立真的 iOS OpenGL ES drawable，並呼叫 libunity JNI_OnLoad 捕捉這版 UnityPlayer 的 native lifecycle entrypoints。"
        } else {
    ...
    @objc private func runGraphicsBridge() {
        ...
        let alert = UIAlertController(
            title: "Unity 圖形橋 0.3",
            message: "這會建立真正的 CAEAGLLayer + EAGLContext drawable，讓 EGL shim 不再只是假的 token；接著載入完整 libunity.so 並呼叫它的 JNI_OnLoad，抓出 UnityPlayer 實際註冊的 native entrypoints。\n\n這一步仍不會直接呼叫 nativeRender，因為 Activity / UnityPlayer lifecycle 還沒建立完整。",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: "開始", style: .destructive) { [weak self] _ in
    ...
    private func performGraphicsBridge() {
        ...
        UserDefaults.standard.synchronize()
        setButtons(enabled: false)
        textView.text = "✅ iOS OpenGL ES drawable 已 present\n\nLoading libunity.so + capturing JNI RegisterNatives…"
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    self.setButtons(enabled: true)
                    self.showMessage(title: "Unity 圖形橋 Probe 失敗", message: error.localizedDescription)
                }
            }
        }
    }
