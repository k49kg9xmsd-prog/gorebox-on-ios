...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "GraphicsProbePending") {
            ...
            showMessage(title: "上一個 Unity Constructor / 圖形 Probe 未完成", message: "最後 checkpoint：\n\n\(cp)\n\n0.3.3 改成批次 Android ABI 相容層：TLS pthread key、mutex/cond/once、semaphore、signal、Linux mmap flags、Android 4K guest page、syscall、dlopen/dlsym 都先攔截；若被終止，這裡會顯示 constructor 或 compatibility bridge checkpoint。")
        }
    }
    ...
    @objc private func runGraphicsBridge() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "Unity Constructors + 圖形橋 0.3.3",
