...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    private let graphicsButton = UIButton(type: .system)
    private let launchButton = UIButton(type: .system)
    private let stopLaunchButton = UIButton(type: .system)
    private let surfaceView = GoreBoxSurfaceView()
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "BootstrapPending") {
            let diskCheckpoint = (try? String(contentsOf: ImportedGame.bootstrapCheckpointURL, encoding: .utf8))?.trimmingCharacters(in: .whitespacesAndNewlines)
            let cp = (diskCheckpoint?.isEmpty == false ? diskCheckpoint : nil) ?? UserDefaults.standard.string(forKey: "BootstrapCheckpoint") ?? "unknown"
            UserDefaults.standard.set(false, forKey: "BootstrapPending")
            showMessage(title: "上一個 Bootstrap 未完成", message: "上次被終止時最後 checkpoint：\n\n\(cp)\n\n0.2.2 不再嘗試 RWX；W+X collision 一律凍結成 RX，並會 flush ARM64 instruction cache。JNI callback 內部也會直接 fsync checkpoint 到磁碟。")
        }
        if UserDefaults.standard.bool(forKey: "GraphicsProbePending") {
            let cp = (try? String(contentsOf: ImportedGame.bootstrapCheckpointURL, encoding: .utf8))?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "unknown"
            UserDefaults.standard.set(false, forKey: "GraphicsProbePending")
            showMessage(title: "上一個 Unity Constructor / 圖形 Probe 未完成", message: "最後 checkpoint：\n\n\(cp)\n\n0.3.6 改成 binary-level TLS virtualization：不再改寫 iOS 真正的 TPIDR_EL0，而是在載入 Android ELF 時，把 guest 的 MRS TPIDR_EL0 指令改成讀取鄰近的 synthetic Bionic TLS page。這能避免 guest 呼叫 Darwin/libSystem 時帶著錯誤 host TLS。")
        }
        if UserDefaults.standard.bool(forKey: "Launch04Pending") {
            let cp = (try? String(contentsOf: ImportedGame.bootstrapCheckpointURL, encoding: .utf8))?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "unknown"
            let trace = readLaunchTrace()
            UserDefaults.standard.set(false, forKey: "Launch04Pending")
            showMessage(
                title: "上一個 GoreBox Launch 0.4.3 未完成",
                message: "最後 checkpoint：\n\n\(cp)\n\n這版已把整段啟動流程逐步寫入 launch-trace.txt。點『複製完整紀錄』後直接貼給我，我可以看到 crash 前走過的全部 checkpoint。",
                copyText: trace.isEmpty ? "Last checkpoint:\n\(cp)" : trace
            )
        }
    }
    ...
    private func configureUI() {
        ...
        styleButton(bootstrapButton, title: "ELF / JNI Bootstrap", action: #selector(runBootstrap))
        bootstrapButton.tintColor = .systemGreen
        styleButton(graphicsButton, title: "Unity Constructors + 圖形橋測試", action: #selector(runGraphicsBridge))
        graphicsButton.tintColor = .systemGreen
        styleButton(launchButton, title: "▶︎ 啟動 GoreBox（實驗性 0.4.3）", action: #selector(runLaunch04))
        launchButton.tintColor = .systemGreen
        styleButton(stopLaunchButton, title: "停止 Unity Render Loop", action: #selector(stopLaunch04))
        stopLaunchButton.tintColor = .systemOrange
        stopLaunchButton.isHidden = true
        ...
        surfaceView.heightAnchor.constraint(equalToConstant: 320).isActive = true
        ...
        let stack = UIStackView(arrangedSubviews: [profileLabel, detailLabel, progress, importButton, diagnosticsButton, guestProbeButton, bootstrapButton, graphicsButton, launchButton, stopLaunchButton, surfaceView, removeButton])
        stack.axis = .vertical
        stack.spacing = 10
    ...
    private func refreshInstalledState() {
        ...
        graphicsButton.isEnabled = installed
        launchButton.isEnabled = installed
        stopLaunchButton.isEnabled = installed
        ...
        if installed, let meta = ImportedGame.metadata() {
            ...
            textView.text = "遊戲已安裝。\n\n0.3.6 已經在實機跨過 Unity constructors、JNI_OnLoad 並抓到 nativeRender。Launch 0.4.3 會進一步預載原版 libil2cpp / RayFire，建立 guest dlopen/dlsym registry，然後真的呼叫 nativeRecreateGfxState → nativeResume → nativeRender。"
        } else {
    ...
    @objc private func runBootstrap() {
        ...
        let alert = UIAlertController(
            title: "實驗性啟動 GoreBox",
            message: "0.2.2 會完全遵守 W^X：Android 4K→iOS 16K 的 W+X collision page 不再嘗試 RWX，而是 relocation 後直接 RX，並 flush ARM64 instruction cache。\n\n在 JNI_OnLoad 前還會先執行完整 mapped libmain 裡的原生 RET sanity probe；每個 JNI callback 都會 fsync checkpoint。若仍出錯，App 仍可能被系統關閉。",
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if allLoaded && report.libmainMappedRetReturned && report.libmainJNIResult == 0x00010006 && report.rayFireFullImageReturned {
                        self.showMessage(
                            title: "底層啟動鏈又過兩關 😆",
    ...
    @objc private func runGraphicsBridge() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "Unity Constructors + 圖形橋 0.3.6",
            message: "這會建立真正的 CAEAGLLayer + EAGLContext drawable，載入完整 libunity.so，然後按照 Android linker 順序執行 DT_INIT + 423 個 DT_INIT_ARRAY constructors，再呼叫 JNI_OnLoad。\n\n每個 constructor 前都會 fsync checkpoint；如果 LiveContainer 終止它，重新開 App 就能看到精確死在哪一個。",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: "開始", style: .destructive) { [weak self] _ in
            self?.performGraphicsBridge()
        })
        present(alert, animated: true)
    }

    private func performGraphicsBridge() {
        surfaceView.isHidden = false
        view.layoutIfNeeded()
        let scale = view.window?.screen.nativeScale ?? UIScreen.main.nativeScale
        let w = Int32(max(surfaceView.bounds.width * scale, 1))
        let h = Int32(max(surfaceView.bounds.height * scale, 1))
        let layerPtr = Unmanaged.passUnretained(surfaceView.layer).toOpaque()
        gbr_ios_gles_attach_layer(layerPtr, w, h)
        let glOK = gbr_ios_gles_self_test() != 0
        if !glOK {
            showMessage(title: "OpenGL ES drawable 建立失敗", message: "CAEAGLLayer/EAGLContext 沒有成功 present。先不要進 Unity JNI。")
            return
        }

        UserDefaults.standard.set(true, forKey: "GraphicsProbePending")
        UserDefaults.standard.synchronize()
        setButtons(enabled: false)
        textView.text = "✅ iOS OpenGL ES drawable 已 present\n\nLoading libunity.so → DT_INIT/DT_INIT_ARRAY → JNI_OnLoad…"

        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let self else { return }
            do {
                let report = try UnityGraphicsProbe.run(graphicsSelfTest: glOK)
                UserDefaults.standard.set(false, forKey: "GraphicsProbePending")
                let text = report.text
                DispatchQueue.main.async {
                    self.latestReport = text
                    self.textView.text = text
                    self.setButtons(enabled: true)
                    let names = Set(report.natives.map(\.name))
                    if report.unityJNIResult == 0x00010006 && names.contains("nativeRender") {
                        self.showMessage(title: "Unity 真入口抓到了 😆", message: "iOS drawable 已能 present，而且 libunity JNI_OnLoad 成功。nativeRender 也已從 GoreBox 的 Unity 註冊表直接抓到。\n\n下一步就是建立 UnityPlayer/Activity lifecycle，再安全地接 nativeRecreateGfxState → nativeResume → nativeRender。")
                    } else {
                        self.showMessage(title: "圖形橋有結果", message: "請把下方報告貼給我。這版會列出實際捕捉到的 Unity JNI native methods。")
                    }
                }
            } catch {
                UserDefaults.standard.set(false, forKey: "GraphicsProbePending")
                DispatchQueue.main.async {
                    self.setButtons(enabled: true)
                    self.showMessage(title: "Unity Constructor / 圖形橋 Probe 失敗", message: error.localizedDescription)
                }
            }
        }
    }

    @objc private func runLaunch04() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "GoreBoxRunner Launch 0.4.3",
            message: "這次不是 Probe：Runner 會保留 libmain / RayFire / libil2cpp / libunity 的完整 mapped ELF，讓 Android dlopen/dlsym 能找到真 guest exports，然後直接呼叫 GoreBox UnityPlayer 註冊的 nativeRecreateGfxState → nativeResume → nativeRender。\n\n第一次會另外從 APK 解出 assets/bin/Data（約 259 MB）到 App 資料中。這仍是實驗性 runtime；如果下一個 Android/JNI ABI 還沒補到，LiveContainer 可能終止 App，但 0.4 會留下精確 checkpoint。",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: "真的啟動", style: .destructive) { [weak self] _ in
            self?.performLaunch04()
        })
        present(alert, animated: true)
    }

    private func performLaunch04() {
        surfaceView.isHidden = false
        view.layoutIfNeeded()
        let scale = view.window?.screen.nativeScale ?? UIScreen.main.nativeScale
        let w = Int32(max(surfaceView.bounds.width * scale, 1))
        ...
        let layerPtr = Unmanaged.passUnretained(surfaceView.layer).toOpaque()
        gbr_ios_gles_attach_layer(layerPtr, w, h)
        guard gbr_ios_gles_self_test() != 0 else {
            showMessage(title: "Launch 0.4.3：drawable 失敗", message: "iOS CAEAGLLayer / EAGLContext 沒有成功 present，先不進 Unity lifecycle。")
            return
        }

        UserDefaults.standard.set(true, forKey: "Launch04Pending")
        UserDefaults.standard.synchronize()
        setButtons(enabled: false)
        progress.isHidden = false
        progress.progress = 0
        textView.text = "Launch 0.4.3 starting…\n\nPreparing APK runtime assets and persistent Android ELF images…"

        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let self else { return }
            do {
                let report = try UnityLaunchRuntime.shared.prepareAndLaunch { message, value in
                    "UI progress \(Int(value * 100))%: \(message)".withCString { gbr_checkpoint_now($0) }
                    DispatchQueue.main.async {
                        self.progress.progress = Float(value)
                        self.textView.text = message
                    }
                ...
                DispatchQueue.main.async {
                    ...
                    self.progress.isHidden = true
                    self.setButtons(enabled: true)
                    self.stopLaunchButton.isHidden = !report.continuousRenderStarted
                    if report.firstRenderResult != nil {
                        let trace = self.readLaunchTrace()
                        self.showMessage(title: "nativeRender 已經真的叫到了 😆", message: "Launch 0.4.3 已經跨過 persistent libil2cpp + UnityPlayer lifecycle，第一個 nativeRender 也有返回。\n\n如果上面的 drawable 開始出現 GoreBox/Unity 畫面，就是第一幀真的到了；如果仍黑畫面，按『複製完整紀錄』貼給我。", copyText: trace + "\n\n--- REPORT ---\n" + text)
                    } else {
                        let trace = self.readLaunchTrace()
                        self.showMessage(title: "Launch 0.4.3 有結果", message: "還沒有抵達第一個 nativeRender。按『複製完整紀錄』把整段流程貼給我。", copyText: trace + "\n\n--- REPORT ---\n" + text)
                    }
                }
            } catch {
                UserDefaults.standard.set(false, forKey: "Launch04Pending")
                DispatchQueue.main.async {
                    ...
                    self.setButtons(enabled: true)
                    let trace = self.readLaunchTrace()
                    self.showMessage(title: "GoreBox Launch 0.4.3 停住", message: error.localizedDescription + "\n\n按『複製完整紀錄』把 crash 前的完整流程貼給我。", copyText: trace)
                }
            }
        }
    }

    @objc private func stopLaunch04() {
        UnityLaunchRuntime.shared.stopRenderLoop()
        stopLaunchButton.isHidden = true
        textView.text += "\n\n⏸ Unity render loop stopped by user."
    }
    ...
    @objc private func removeGame() {
        ...
        alert.addAction(UIAlertAction(title: "移除", style: .destructive) { [weak self] _ in
            UnityLaunchRuntime.shared.shutdown()
    ...
    private func setButtons(enabled: Bool) {
        ...
        graphicsButton.isEnabled = enabled && ImportedGame.isInstalled()
        launchButton.isEnabled = enabled && ImportedGame.isInstalled()
        stopLaunchButton.isEnabled = ImportedGame.isInstalled()
        removeButton.isEnabled = enabled && ImportedGame.isInstalled()
    }

    private func readLaunchTrace() -> String {
        (try? String(contentsOf: ImportedGame.launchTraceURL, encoding: .utf8)) ?? ""
    }

    private func showMessage(title: String, message: String, copyText: String? = nil) {
        let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
        if let copyText, !copyText.isEmpty {
            alert.addAction(UIAlertAction(title: "複製完整紀錄", style: .default) { _ in
