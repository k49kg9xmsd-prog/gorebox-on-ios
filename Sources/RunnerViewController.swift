...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "Launch04Pending") {
            ...
            showMessage(
                title: "上一個 GoreBox Launch 0.4.4 未完成",
    ...
    private func configureUI() {
        ...
        styleButton(launchButton, title: "▶︎ 啟動 GoreBox（實驗性 0.4.4）", action: #selector(runLaunch04))
    ...
    private func refreshInstalledState() {
        ...
        if installed, let meta = ImportedGame.metadata() {
            ...
            textView.text = "遊戲已安裝。\n\n0.3.6 已經在實機跨過 Unity constructors、JNI_OnLoad 並抓到 nativeRender。Launch 0.4.4 會進一步預載原版 libil2cpp / RayFire，建立 guest dlopen/dlsym registry，然後真的呼叫 nativeRecreateGfxState → nativeResume → nativeRender。"
        } else {
    ...
    @objc private func runLaunch04() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "GoreBoxRunner Launch 0.4.4",
    ...
    private func performLaunch04() {
        ...
        guard gbr_ios_gles_self_test() != 0 else {
            showMessage(title: "Launch 0.4.4：drawable 失敗", message: "iOS CAEAGLLayer / EAGLContext 沒有成功 present，先不進 Unity lifecycle。")
            return
        }
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if report.firstRenderResult != nil {
                        ...
                        self.showMessage(title: "nativeRender 已經真的叫到了 😆", message: "Launch 0.4.4 已經跨過 persistent libil2cpp + UnityPlayer lifecycle，第一個 nativeRender 也有返回。\n\n如果上面的 drawable 開始出現 GoreBox/Unity 畫面，就是第一幀真的到了；如果仍黑畫面，按『複製完整紀錄』貼給我。", copyText: trace + "\n\n--- REPORT ---\n" + text)
                    } else {
                        let trace = self.readLaunchTrace()
                        self.showMessage(title: "Launch 0.4.4 有結果", message: "還沒有抵達第一個 nativeRender。按『複製完整紀錄』把整段流程貼給我。", copyText: trace + "\n\n--- REPORT ---\n" + text)
                    }
                }
            } catch {
                UserDefaults.standard.set(false, forKey: "Launch04Pending")
                DispatchQueue.main.async {
                    ...
                    self.setButtons(enabled: true)
                    let trace = self.readLaunchTrace()
                    self.showMessage(title: "GoreBox Launch 0.4.4 停住", message: error.localizedDescription + "\n\n按『複製完整紀錄』把 crash 前的完整流程貼給我。", copyText: trace)
                }
            }
        }
    }
