...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    private func configureUI() {
        ...
        styleButton(launchButton, title: "▶︎ 啟動 GoreBox（實驗性 0.4）", action: #selector(runLaunch04))
    ...
    private func refreshInstalledState() {
        ...
        if installed, let meta = ImportedGame.metadata() {
            ...
            textView.text = "遊戲已安裝。\n\n0.3.6 已經在實機跨過 Unity constructors、JNI_OnLoad 並抓到 nativeRender。Launch 0.4 會進一步預載原版 libil2cpp / RayFire，建立 guest dlopen/dlsym registry，然後真的呼叫 nativeRecreateGfxState → nativeResume → nativeRender。"
        } else {
    ...
    @objc private func runLaunch04() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "GoreBoxRunner Launch 0.4",
    ...
    private func performLaunch04() {
        ...
        guard gbr_ios_gles_self_test() != 0 else {
            showMessage(title: "Launch 0.4：drawable 失敗", message: "iOS CAEAGLLayer / EAGLContext 沒有成功 present，先不進 Unity lifecycle。")
            return
        }
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if report.firstRenderResult != nil {
                        self.showMessage(title: "nativeRender 已經真的叫到了 😆", message: "Launch 0.4 已經跨過 persistent libil2cpp + UnityPlayer lifecycle，第一個 nativeRender 也有返回。\n\n如果上面的 drawable 開始出現 GoreBox/Unity 畫面，就是第一幀真的到了；如果仍黑畫面，把下方 report 給我，下一步會看 asset/JNI/EGL 哪一層還沒滿足。")
                    } else {
                        self.showMessage(title: "Launch 0.4 有結果", message: "還沒有抵達第一個 nativeRender。請把下方 report 或重新開 App 後的 checkpoint 給我。")
                    }
                }
            } catch {
                UserDefaults.standard.set(false, forKey: "Launch04Pending")
                DispatchQueue.main.async {
                    self.progress.isHidden = true
                    self.setButtons(enabled: true)
                    self.showMessage(title: "GoreBox Launch 0.4 停住", message: error.localizedDescription)
                }
            }
        }
    }
