...
final class RunnerViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: "GraphicsProbePending") {
            ...
            showMessage(title: "上一個 Unity Constructor / 圖形 Probe 未完成", message: "最後 checkpoint：\n\n\(cp)\n\n0.3.5 新增 Android arm64 TPIDR_EL0/Bionic TLS bridge。0.3.4 卡住的 0xCDCD0 後段會進入直接讀 TPIDR_EL0+0x28 stack guard 的 Unity helper，這種存取無法靠一般 symbol shim 攔截。0.3.5 會在每次 guest constructor/JNI/native call 前安裝 Bionic-shaped TLS，返回 iOS 後恢復 host TLS。")
        }
    }
    ...
    private func configureUI() {
        ...
        surfaceView.heightAnchor.constraint(equalToConstant: 220).isActive = true
        ...
        let stack = UIStackView(arrangedSubviews: [profileLabel, detailLabel, progress, importButton, diagnosticsButton, guestProbeButton, bootstrapButton, graphicsButton, surfaceView, removeButton])
        stack.axis = .vertical
        stack.spacing = 10
    ...
    private func refreshInstalledState() {
        ...
        if installed, let meta = ImportedGame.metadata() {
            ...
            textView.text = "遊戲已安裝。\n\n底層 ELF / relocation / libmain JNI_OnLoad 已通過。現在可以按「Unity Constructors + 圖形橋測試」：它會建立真的 iOS OpenGL ES drawable，並呼叫 libunity JNI_OnLoad 捕捉這版 UnityPlayer 的 native lifecycle entrypoints。"
        } else {
    ...
    @objc private func runGraphicsBridge() {
        guard ImportedGame.isInstalled() else { return }
        let alert = UIAlertController(
            title: "Unity Constructors + 圖形橋 0.3.5",
