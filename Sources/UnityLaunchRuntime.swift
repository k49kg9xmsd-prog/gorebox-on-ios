...
struct UnityLaunchReport {
    ...
    var text: String {
        ...
        out.append("")
        out.append("Launch 0.4 is the first build that actually calls the real GoreBox UnityPlayer lifecycle entrypoints. It still uses a compatibility Activity/JNI surface, so success here means first-frame/runtime progress — not yet a guarantee that touch/audio/save systems are playable.")
...
final class UnityLaunchRuntime {
    ...
    func prepareAndLaunch(progress: @escaping (String, Double) -> Void) throws -> UnityLaunchReport {
        ...
        launchCheckpoint("Launch 0.4: extracting/checking Unity runtime assets")
        ...
        launchCheckpoint("Launch 0.4: iOS drawable is current")
        ...
        for (index, item) in order.enumerated() {
            ...
            launchCheckpoint("Launch 0.4: mapping \(item.0)")
        ...
        launchCheckpoint("Launch 0.4: about to CALL nativeRecreateGfxState(0, Surface)")
        ...
        launchCheckpoint("Launch 0.4: about to CALL first nativeRender")
        _ = gbr_ios_gles_make_current()
        let first = gbr_jni_call_native_render(render)
        launchCheckpoint("Launch 0.4: first nativeRender returned")
            ...
            while true {
                ...
                autoreleasepool {
                    ...
                    if self.frameCounter <= 5 || self.frameCounter % 120 == 0 {
                        launchCheckpoint("Launch 0.4: nativeRender loop frame \(self.frameCounter)")
                    }
