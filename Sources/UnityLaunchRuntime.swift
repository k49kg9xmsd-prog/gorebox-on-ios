...
final class UnityLaunchRuntime {
    ...
    func prepareAndLaunch(progress: @escaping (String, Double) -> Void) throws -> UnityLaunchReport {
        ...
        for (index, item) in order.enumerated() {
            ...
            launchCheckpoint("Launch 0.4.3: mapping \(item.0)")
        ...
        launchCheckpoint("Launch 0.4.3: about to CALL nativeRecreateGfxState(0, Surface)")
        _ = gbr_jni_call_native_recreate_gfx_state(recreate, 0)
        launchCheckpoint("Launch 0.4.3: nativeRecreateGfxState RETURNED")
        if let surfaceChanged {
            launchCheckpoint("Launch 0.4.3: about to CALL nativeSendSurfaceChangedEvent")
            gbr_jni_call_native_surface_changed(surfaceChanged)
            launchCheckpoint("Launch 0.4.3: nativeSendSurfaceChangedEvent RETURNED")
        }
        if let focus {
            launchCheckpoint("Launch 0.4.3: about to CALL nativeFocusChanged(1)")
            gbr_jni_call_native_focus_changed(focus, 1)
            launchCheckpoint("Launch 0.4.3: nativeFocusChanged RETURNED")
        }

        progress("Calling real UnityPlayer nativeResume…", 0.86)
        launchCheckpoint("Launch 0.4.3: about to CALL nativeResume")
        gbr_jni_call_native_resume(resume)
        launchCheckpoint("Launch 0.4.3: nativeResume RETURNED")

        progress("Calling first real UnityPlayer nativeRender frame…", 0.94)
        launchCheckpoint("Launch 0.4.3: about to CALL first nativeRender")
        _ = gbr_ios_gles_make_current()
        let first = gbr_jni_call_native_render(render)
        launchCheckpoint("Launch 0.4.3: first nativeRender returned")
