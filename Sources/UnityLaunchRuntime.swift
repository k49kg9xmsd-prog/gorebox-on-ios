...
final class UnityLaunchRuntime {
    ...
    func prepareAndLaunch(progress: @escaping (String, Double) -> Void) throws -> UnityLaunchReport {
        ...
        for (index, item) in order.enumerated() {
            ...
            launchCheckpoint(String(format: "Launch 0.4.4: %@ map=%p size=0x%llx load_bias=0x%llx", item.0, image.image.mapping, UInt64(image.image.mapping_size), image.image.load_bias))
            images.append(image)
