...
final class UnityLaunchRuntime {
    ...
    func prepareAndLaunch(progress: @escaping (String, Double) -> Void) throws -> UnityLaunchReport {
        ...
        ImportedGame.launchTraceURL.path.withCString { gbr_set_trace_path($0) }
        gbr_reset_trace_log()
        gbr_install_crash_trace_handlers()
        ...
        launchCheckpoint("Launch 0.4.3: extracting/checking Unity runtime assets")
        ...
        launchCheckpoint("Launch 0.4.3: iOS drawable is current")
        ...
        for (index, item) in order.enumerated() {
            ...
            launchCheckpoint("Launch 0.4.4: mapping \(item.0)")
            try image.mapAndInitialize(registerExports: true, callJNI: item.1)
            let mappingAddress = image.image.mapping.map { UInt64(UInt(bitPattern: $0)) } ?? 0
            launchCheckpoint("Launch 0.4.4: \(item.0) map=0x\(String(mappingAddress, radix: 16)) size=0x\(String(UInt64(image.image.mapping_size), radix: 16)) load_bias=0x\(String(image.image.load_bias, radix: 16))")
            images.append(image)
        ...
        launchCheckpoint(String(format: "Launch 0.4.4: native pointers recreate=0x%llx resume=0x%llx render=0x%llx", UInt64(recreateValue), UInt64(resumeValue), UInt64(renderValue)))
        ...
        launchCheckpoint("Launch 0.4.4: about to CALL nativeRecreateGfxState(0, Surface)")
        _ = gbr_jni_call_native_recreate_gfx_state(recreate, 0)
        launchCheckpoint("Launch 0.4.4: nativeRecreateGfxState RETURNED")
        if let surfaceChanged {
            launchCheckpoint("Launch 0.4.4: about to CALL nativeSendSurfaceChangedEvent")
            gbr_jni_call_native_surface_changed(surfaceChanged)
            launchCheckpoint("Launch 0.4.4: nativeSendSurfaceChangedEvent RETURNED")
        }
        if let focus {
            launchCheckpoint("Launch 0.4.4: about to CALL nativeFocusChanged(1)")
            gbr_jni_call_native_focus_changed(focus, 1)
            launchCheckpoint("Launch 0.4.4: nativeFocusChanged RETURNED")
        }

        progress("Calling real UnityPlayer nativeResume…", 0.86)
        launchCheckpoint("Launch 0.4.4: about to CALL nativeResume")
        gbr_jni_call_native_resume(resume)
        launchCheckpoint("Launch 0.4.4: nativeResume RETURNED")

        progress("Calling first real UnityPlayer nativeRender frame…", 0.94)
        launchCheckpoint("Launch 0.4.4: about to CALL first nativeRender")
        _ = gbr_ios_gles_make_current()
        let first = gbr_jni_call_native_render(render)
        launchCheckpoint("Launch 0.4.4: first nativeRender returned")
            ...
            while true {
                ...
                autoreleasepool {
                    ...
                    if self.frameCounter <= 5 || self.frameCounter % 120 == 0 {
                        launchCheckpoint("Launch 0.4.3: nativeRender loop frame \(self.frameCounter)")
                    }
