...
static void *gbr_guest_thread_trampoline(void *opaque) {
    ...
    GBRGuestTLSContext guest_tls = gbr_guest_tls_enter(tls_block);
    void *result = guest_start(guest_arg);
    gbr_guest_tls_leave(&guest_tls);
    free(tls_block);
    return result;
}
...
int gbr_elf_run_initializers(GBRELFImage *image) {
    ...
    if (image->init_vaddr) {
        ...
        GBRGuestTLSContext guest_tls = gbr_guest_tls_enter(NULL);
        ((void (*)(void))address)();
        gbr_guest_tls_leave(&guest_tls);
...
int32_t gbr_call_fake_jni_onload(void *function_address) {
    ...
    if (!gbr_logged_tpidr_bridge) {
        ...
        gbr_checkpoint_now("Bionic TPIDR_EL0 bridge enabled for guest ARM64 TLS / stack guard");
    }
