...
static char gbr_trace_path[PATH_MAX];
static uint64_t gbr_trace_seq = 0;
...
void gbr_set_trace_path(const char *path) {
    if (!path) {
        gbr_trace_path[0] = '\0';
        return;
    }
    snprintf(gbr_trace_path, sizeof(gbr_trace_path), "%s", path);
}

static void gbr_write_all_fd(int fd, const char *p, size_t left) {
    while (left) {
...
void gbr_reset_trace_log(void) {
    gbr_trace_seq = 0;
    if (!gbr_trace_path[0]) return;
    int fd = open(gbr_trace_path, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    if (fd >= 0) {
        const char *header = "GoreBoxRunner Launch 0.4.4 full trace\n========================================\n";
        gbr_write_all_fd(fd, header, strlen(header));
        (void)fsync(fd);
        close(fd);
    }
}
...
void gbr_checkpoint_now(const char *message) {
    ...
    if (gbr_checkpoint_path[0]) {
        int fd = open(gbr_checkpoint_path, O_WRONLY | O_CREAT | O_TRUNC, 0600);
        if (fd >= 0) {
            gbr_write_all_fd(fd, message, strlen(message));
            (void)fsync(fd);
#ifdef __APPLE__
#ifdef F_FULLFSYNC
    ...
    if (gbr_trace_path[0]) {
        char line[1400];
        unsigned long long seq = (unsigned long long)__sync_add_and_fetch(&gbr_trace_seq, 1);
        int n = snprintf(line, sizeof(line), "[%06llu] %s\n", seq, message);
        if (n < 0) return;
        size_t len = (size_t)n < sizeof(line) ? (size_t)n : sizeof(line) - 1;
        int fd = open(gbr_trace_path, O_WRONLY | O_CREAT | O_APPEND, 0600);
        if (fd >= 0) {
            gbr_write_all_fd(fd, line, len);
            (void)fsync(fd);
#ifdef __APPLE__
#ifdef F_FULLFSYNC
            (void)fcntl(fd, F_FULLFSYNC);
...
// ---------------- Crash-time lifecycle recorder ----------------
// During the real UnityPlayer lifecycle call, a hard SIGSEGV/SIGBUS/SIGILL can
// occur before any imported Android symbol/JNI callback is reached.  Keep a tiny
// async-signal-safe recorder so the next launch can show the exact fault PC/LR.
static volatile sig_atomic_t gbr_lifecycle_trace_active = 0;
static volatile sig_atomic_t gbr_last_bridge_id = 0;
static struct sigaction gbr_old_segv, gbr_old_bus, gbr_old_ill, gbr_old_abrt;
static int gbr_crash_handlers_installed = 0;

void gbr_set_lifecycle_trace_active(int32_t active) {
    gbr_lifecycle_trace_active = active ? 1 : 0;
    if (!active) gbr_last_bridge_id = 0;
}

static size_t gbr_sig_append_text(char *dst, size_t cap, size_t pos, const char *src) {
    if (!dst || !src || pos >= cap) return pos;
    while (*src && pos + 1 < cap) dst[pos++] = *src++;
    return pos;
}
static size_t gbr_sig_append_hex(char *dst, size_t cap, size_t pos, uintptr_t v) {
    static const char hex[] = "0123456789abcdef";
    pos = gbr_sig_append_text(dst, cap, pos, "0x");
    int started = 0;
    for (int shift=(int)(sizeof(uintptr_t)*8)-4; shift>=0; shift-=4) {
        unsigned d=(unsigned)((v >> shift) & 0xfu);
        if (!started && d==0 && shift>0) continue;
        started=1;
        if (pos + 1 < cap) dst[pos++]=hex[d];
    }
    return pos;
}
static size_t gbr_sig_append_uint(char *dst, size_t cap, size_t pos, unsigned v) {
...
static void gbr_crash_signal_handler(int sig, siginfo_t *info, void *uctx) {
    if (gbr_lifecycle_trace_active && gbr_trace_path[0]) {
        ...
        p=gbr_sig_append_text(line,sizeof(line),p,"[CRASH] signal=");
        p=gbr_sig_append_uint(line,sizeof(line),p,(unsigned)sig);
        p=gbr_sig_append_text(line,sizeof(line),p," fault=");
        p=gbr_sig_append_hex(line,sizeof(line),p,(uintptr_t)(info?info->si_addr:0));
        p=gbr_sig_append_text(line,sizeof(line),p," pc=");
        p=gbr_sig_append_hex(line,sizeof(line),p,pc);
        p=gbr_sig_append_text(line,sizeof(line),p," lr=");
        p=gbr_sig_append_hex(line,sizeof(line),p,lr);
        p=gbr_sig_append_text(line,sizeof(line),p," sp=");
        p=gbr_sig_append_hex(line,sizeof(line),p,sp);
        p=gbr_sig_append_text(line,sizeof(line),p," lastBridge=");
        p=gbr_sig_append_uint(line,sizeof(line),p,(unsigned)gbr_last_bridge_id);
        if (p+1<sizeof(line)) line[p++]='\n';
        int fd=open(gbr_trace_path,O_WRONLY|O_CREAT|O_APPEND,0600);
        if(fd>=0){gbr_write_all_fd(fd,line,p);(void)fsync(fd);close(fd);}
        if (gbr_checkpoint_path[0]) {
            int cfd=open(gbr_checkpoint_path,O_WRONLY|O_CREAT|O_TRUNC,0600);
            if(cfd>=0){gbr_write_all_fd(cfd,line,p);(void)fsync(cfd);close(cfd);}
        }
    }
    signal(sig, SIG_DFL);
    raise(sig);
...
void gbr_install_crash_trace_handlers(void) {
    if (gbr_crash_handlers_installed) return;
    struct sigaction sa; memset(&sa,0,sizeof(sa));
    sa.sa_sigaction=gbr_crash_signal_handler; sigemptyset(&sa.sa_mask);
    sa.sa_flags=SA_SIGINFO|SA_RESETHAND;
    sigaction(SIGSEGV,&sa,&gbr_old_segv);
    sigaction(SIGBUS,&sa,&gbr_old_bus);
    sigaction(SIGILL,&sa,&gbr_old_ill);
    sigaction(SIGABRT,&sa,&gbr_old_abrt);
    gbr_crash_handlers_installed=1;
}
...
static int gbr_guest_pthread_mutex_unlock(void *guest) {
    gbr_last_bridge_id=102; GBRMutexSlot *s=gbr_mutex_slot(guest,1); return s ? pthread_mutex_unlock(&s->host) : EINVAL; }
...
static void *gbr_egl_get_display(void *native) { gbr_last_bridge_id=301; (void)native; gbr_checkpoint_now("EGL: eglGetDisplay entered"); return &gbr_egl_display_token; }
static unsigned int gbr_egl_initialize(void *d, int *major, int *minor) { gbr_last_bridge_id=302; gbr_checkpoint_now("EGL: eglInitialize entered"); if (!d) { gbr_egl_last_error=0x3008; return 0; } if (major) *major=1; if (minor) *minor=5; gbr_egl_last_error=0x3000; return 1; }
...
static void *gbr_egl_create_context(void *d, void *cfg, void *share, const int *attrs) { gbr_last_bridge_id=303; (void)d; (void)cfg; (void)share; (void)attrs; gbr_checkpoint_now("EGL: eglCreateContext entered"); if (!gbr_ios_gles_create_context()) { gbr_egl_last_error=0x3003; return NULL; } return &gbr_egl_context_token; }
static void *gbr_egl_create_window_surface(void *d, void *cfg, void *win, const int *attrs) { gbr_last_bridge_id=304; (void)d; (void)cfg; (void)win; (void)attrs; gbr_checkpoint_now("EGL: eglCreateWindowSurface entered"); if (!gbr_ios_gles_create_window_surface()) { gbr_egl_last_error=0x3003; return NULL; } return &gbr_egl_surface_token; }
...
int32_t gbr_jni_call_native_recreate_gfx_state(void *function_address,int32_t display_index){
    ...
    char cp[256]; snprintf(cp,sizeof(cp),"Launch 0.4.4: entering nativeRecreateGfxState fn=%p env=%p player=%p surface=%p",function_address,(void*)&gbr_env_obj,gbr_fake_unity_player(),gbr_fake_surface_object());
    gbr_checkpoint_now(cp);
    gbr_last_bridge_id=1; gbr_set_lifecycle_trace_active(1);
    ((Fn)function_address)(&gbr_env_obj,gbr_fake_unity_player(),display_index,gbr_fake_surface_object());
    gbr_set_lifecycle_trace_active(0);
    gbr_checkpoint_now("Launch 0.4.4: nativeRecreateGfxState returned");return 0;
}
