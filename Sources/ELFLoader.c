...
void gbr_reset_trace_log(void) {
    ...
    if (fd >= 0) {
        const char *header = "GoreBoxRunner Launch 0.4.3 full trace\n========================================\n";
...
int32_t gbr_jni_call_native_recreate_gfx_state(void *function_address,int32_t display_index){
    ...
    gbr_checkpoint_now("Launch 0.4.3: entering nativeRecreateGfxState (correct Bionic jmp_buf layout)");
    ((Fn)function_address)(&gbr_env_obj,gbr_fake_unity_player(),display_index,gbr_fake_surface_object());
    gbr_checkpoint_now("Launch 0.4.3: nativeRecreateGfxState returned");return 0;
}
