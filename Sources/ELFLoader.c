...
#define EQ(x) (strcmp(name, (x)) == 0)
    ...
    if (strncmp(name, "ANativeWindow_", 14) == 0) {
        ...
        if (EQ("ANativeWindow_fromSurface")) { gbr_checkpoint_now("ANativeWindow_fromSurface -> iOS drawable token"); return (void *)&gbr_stub_ptr; }
...
int32_t gbr_jni_call_native_recreate_gfx_state(void *function_address,int32_t display_index){
    ...
    gbr_checkpoint_now("Launch 0.4: entering nativeRecreateGfxState");
    ((Fn)function_address)(&gbr_env_obj,gbr_fake_unity_player(),display_index,gbr_fake_surface_object());
    gbr_checkpoint_now("Launch 0.4: nativeRecreateGfxState returned");return 0;
}
