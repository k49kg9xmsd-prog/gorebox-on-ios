...
struct GoreBoxBootstrapReport {
    ...
    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Bootstrap 0.2.1")
        ...
        for r in libraries {
            ...
            if r.rwxPages > 0 { out.append("RWX host pages accepted: \(r.rwxPages)") }
            if r.rxFallbackPages > 0 { out.append("W^X fallback pages forced RX: \(r.rxFallbackPages) ⚠️ bootstrap-only") }
...
enum BootstrapLoader {
    static func run(progress: @escaping (String, Float) -> Void) throws -> GoreBoxBootstrapReport {
        ...
        return GoreBoxBootstrapReport(libraries: results, libmainJNIResult: mainResult, rayFireFullImageReturned: rayReturned)
    }
}
