...
struct UnityGraphicsProbeReport {
    ...
    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Graphics Bridge 0.3.4")
        ...
        out.append("")
        out.append("0.3.4 is the constructor pre-scan pass: in addition to the 0.3.3 pthread/TLS/semaphore/syscall bridges, Bionic FILE*/__sF stdio calls are intercepted, Android clock IDs are translated, Linux open flags are translated without consuming guest varargs, and guest __cxa_atexit/__cxa_finalize no longer hand raw Android function pointers to Darwin. If the process is terminated, reopen GoreBoxRunner; the durable checkpoint identifies the constructor or compatibility bridge.")
...
enum UnityGraphicsProbe {
    static func run(graphicsSelfTest: Bool) throws -> UnityGraphicsProbeReport {
        ...
        gbr_checkpoint_now("graphics 0.3.4: before libunity ELF load")
        ...
        try data.withUnsafeBytes { raw in
            ...
            gbr_checkpoint_now("graphics 0.3.4: libunity ELF mapped + relocated")
            guard loadCode == 0 else { return }

            gbr_checkpoint_now("graphics 0.3.4: about to execute libunity DT_INIT/DT_INIT_ARRAY")
            ...
            gbr_checkpoint_now("graphics 0.3.4: libunity initializers completed")

            guard let onLoad = gbr_elf_find_symbol(base, data.count, &image, "JNI_OnLoad") else {
                throw UnityGraphicsProbeError.jniMissing
            }
            gbr_checkpoint_now("graphics 0.3.4: about to CALL libunity JNI_OnLoad AFTER constructors")
            jniResult = gbr_call_fake_jni_onload(onLoad)
            gbr_checkpoint_now("graphics 0.3.4: returned from libunity JNI_OnLoad")
        }
        ...
        gbr_checkpoint_now("graphics 0.3.4: completed")
        return report
    }
}
