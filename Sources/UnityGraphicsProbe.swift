...
struct UnityGraphicsProbeReport {
    ...
    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Graphics Bridge 0.3.5")
        ...
        out.append("")
        out.append("0.3.5 adds a Bionic TPIDR_EL0 TLS context around every guest constructor/JNI/native call. GoreBox libunity contains direct Android arm64 TLS reads that bypass imported-symbol shims; the 0xCDCD0 constructor reaches one of these stack-protected helpers. Existing pthread/TLS/semaphore/syscall/stdio/open/C++ bridges remain enabled. If the process is terminated, reopen GoreBoxRunner to read the durable checkpoint.")
...
enum UnityGraphicsProbe {
    static func run(graphicsSelfTest: Bool) throws -> UnityGraphicsProbeReport {
        ...
        gbr_checkpoint_now("graphics 0.3.5: before libunity ELF load")
        ...
        try data.withUnsafeBytes { raw in
            ...
            gbr_checkpoint_now("graphics 0.3.5: libunity ELF mapped + relocated")
            guard loadCode == 0 else { return }

            gbr_checkpoint_now("graphics 0.3.5: about to execute libunity DT_INIT/DT_INIT_ARRAY")
            ...
            gbr_checkpoint_now("graphics 0.3.5: libunity initializers completed")

            guard let onLoad = gbr_elf_find_symbol(base, data.count, &image, "JNI_OnLoad") else {
                throw UnityGraphicsProbeError.jniMissing
            }
            gbr_checkpoint_now("graphics 0.3.5: about to CALL libunity JNI_OnLoad AFTER constructors")
            jniResult = gbr_call_fake_jni_onload(onLoad)
            gbr_checkpoint_now("graphics 0.3.5: returned from libunity JNI_OnLoad")
        }
        ...
        gbr_checkpoint_now("graphics 0.3.5: completed")
        return report
    }
}
