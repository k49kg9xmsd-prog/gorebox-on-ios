...
struct UnityGraphicsProbeReport {
    ...
    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Graphics Bridge 0.3.2")
        ...
        out.append("")
        out.append("0.3.2 executes DT_INIT / DT_INIT_ARRAY and adds a Bionic pthread synchronization bridge plus Android-arm64 syscall translation. Android pthread objects are no longer passed directly to Darwin, and Linux syscall numbers are never forwarded blindly to the iOS kernel. If the process is terminated, reopen GoreBoxRunner; the durable checkpoint identifies the constructor or compatibility call.")
...
enum UnityGraphicsProbe {
    static func run(graphicsSelfTest: Bool) throws -> UnityGraphicsProbeReport {
        ...
        gbr_checkpoint_now("graphics 0.3.2: before libunity ELF load")
        ...
        try data.withUnsafeBytes { raw in
            ...
            gbr_checkpoint_now("graphics 0.3.2: libunity ELF mapped + relocated")
            guard loadCode == 0 else { return }

            gbr_checkpoint_now("graphics 0.3.2: about to execute libunity DT_INIT/DT_INIT_ARRAY")
            ...
            gbr_checkpoint_now("graphics 0.3.2: libunity initializers completed")

            guard let onLoad = gbr_elf_find_symbol(base, data.count, &image, "JNI_OnLoad") else {
                throw UnityGraphicsProbeError.jniMissing
            }
            gbr_checkpoint_now("graphics 0.3.2: about to CALL libunity JNI_OnLoad AFTER constructors")
            jniResult = gbr_call_fake_jni_onload(onLoad)
            gbr_checkpoint_now("graphics 0.3.2: returned from libunity JNI_OnLoad")
        }
        ...
        gbr_checkpoint_now("graphics 0.3.2: completed")
        return report
    }
}
