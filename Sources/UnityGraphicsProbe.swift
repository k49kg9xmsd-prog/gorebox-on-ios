...
struct UnityGraphicsProbeReport {
    ...
    let tpidrPatchCount: Int
    let natives: [RegisteredUnityNative]

    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Graphics Bridge 0.3.6")
        ...
        out.append("Android TPIDR_EL0 reads virtualized: \(tpidrPatchCount)")
        ...
        out.append("")
        out.append("0.3.6 no longer overwrites iOS TPIDR_EL0. During ELF load it rewrites Android `MRS TPIDR_EL0` instructions to a nearby synthetic Bionic TLS page, so guest stack/TLS reads work while Darwin/libSystem keeps its real host thread pointer. Existing pthread/semaphore/syscall/stdio/open/C++ bridges remain enabled. If the process is terminated, reopen GoreBoxRunner to read the durable checkpoint.")
...
enum UnityGraphicsProbe {
    static func run(graphicsSelfTest: Bool) throws -> UnityGraphicsProbeReport {
        ...
        gbr_checkpoint_now("graphics 0.3.6: before libunity ELF load")
        ...
        try data.withUnsafeBytes { raw in
            ...
            gbr_checkpoint_now("graphics 0.3.6: libunity ELF mapped + relocated")
            guard loadCode == 0 else { return }

            gbr_checkpoint_now("graphics 0.3.6: about to execute libunity DT_INIT/DT_INIT_ARRAY")
            ...
            gbr_checkpoint_now("graphics 0.3.6: libunity initializers completed")

            guard let onLoad = gbr_elf_find_symbol(base, data.count, &image, "JNI_OnLoad") else {
                throw UnityGraphicsProbeError.jniMissing
            }
            gbr_checkpoint_now("graphics 0.3.6: about to CALL libunity JNI_OnLoad AFTER constructors")
            jniResult = gbr_call_fake_jni_onload(onLoad)
            gbr_checkpoint_now("graphics 0.3.6: returned from libunity JNI_OnLoad")
        }
        ...
        let report = UnityGraphicsProbeReport(
            ...
            tpidrPatchCount: Int(image.tpidr_patch_count),
            natives: captured
        )
        gbr_elf_unload_image(&image)
        gbr_checkpoint_now("graphics 0.3.6: completed")
        return report
    }
}
