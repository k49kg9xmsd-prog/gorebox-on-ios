...
struct UnityGraphicsProbeReport {
    ...
    var text: String {
        var out: [String] = []
        out.append("GoreBoxRunner Graphics Bridge 0.3.1")
        ...
        out.append("")
        out.append("0.3.1 now reproduces the missing Android linker step: DT_INIT / DT_INIT_ARRAY are executed before JNI_OnLoad. If the process is terminated, reopen GoreBoxRunner; the durable checkpoint reports the exact constructor ordinal and guest address that was about to run.")
...
enum UnityGraphicsProbe {
    static func run(graphicsSelfTest: Bool) throws -> UnityGraphicsProbeReport {
        ...
        gbr_checkpoint_now("graphics 0.3.1: before libunity ELF load")
        ...
        try data.withUnsafeBytes { raw in
            ...
            gbr_checkpoint_now("graphics 0.3.1: libunity ELF mapped + relocated")
            guard loadCode == 0 else { return }

            gbr_checkpoint_now("graphics 0.3.1: about to execute libunity DT_INIT/DT_INIT_ARRAY")
            ...
            gbr_checkpoint_now("graphics 0.3.1: libunity initializers completed")

            guard let onLoad = gbr_elf_find_symbol(base, data.count, &image, "JNI_OnLoad") else {
                throw UnityGraphicsProbeError.jniMissing
            }
            gbr_checkpoint_now("graphics 0.3.1: about to CALL libunity JNI_OnLoad AFTER constructors")
            jniResult = gbr_call_fake_jni_onload(onLoad)
            gbr_checkpoint_now("graphics 0.3.1: returned from libunity JNI_OnLoad")
        }
        ...
        gbr_checkpoint_now("graphics 0.3.1: completed")
        return report
    }
}
